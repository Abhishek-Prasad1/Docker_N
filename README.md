...
In other words, a container is a bundle of Application, Application libraries required to run your application and the minimum system dependencies.

# Containers vs Virtual Machine

<img width="1662" height="718" alt="image" src="https://github.com/user-attachments/assets/efd17ae4-a92b-4e9c-bb33-0589d402f822" />
...
Improved Security: A smaller image size reduces the "attack surface" because unnecessary packages, shells, and tools are not present in the production container.
Better Maintainability: You manage the entire build process in a single Dockerfile instead of using separate Dockerfiles and auxiliary shell scripts (which was the common "builder pattern" before this feature existed).
Faster Deployment: Smaller images lead to faster transfer times across networks and quicker container startup times. 

#Distroless image:
A Distroless image is a minimal container image that contains only your application and its runtime dependencies—no OS package manager, no shell, no utilities like bash, apt, yum, etc.
It’s mainly used in production to improve security, performance, and image size.
